...
def parsear(filas):
    ...
    for i in range(1, int(len(list(filas))/3)):
        ...
        print(filas[3*i].text)
        print("--ternino de impriir fecha--")
        dic['bid'] = filas[3*i+1].text[2:]
        print(filas[3*i+1].text)
        print("--ternino de impriir bid--")
        dic['ask'] = filas[3*i+2].text[2:]
        print(filas[3*i+2].text)
        print("--ternino de impriir ask--")
